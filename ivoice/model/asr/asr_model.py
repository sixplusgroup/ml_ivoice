...
from ivoice.model.asr.global_cmvn import GlobalCMVN
from ivoice.model.asr.ctc import CTC
from ivoice.model.asr.decoder import (TransformerDecoder, BiTransformerDecoder)
from ivoice.model.asr.encoder import (ConformerEncoder, TransformerEncoder)
from ivoice.model.asr.label_smoothing_loss import LabelSmoothingLoss
from ivoice.util.cmvn import load_cmvn
from ivoice.util.common import (add_sos_eos, log_add,
                                remove_duplicates_and_blank, th_accuracy,
                                reverse_pad_list)
from ivoice.util.mask import (make_pad_mask, mask_finished_preds,
...
def init_asr_model(config):
  if config['cmvn_file'] is not None:
    # 均值方差归一化，将特征规范化为均值为0，方差为1的标准特征
    # cmvn_file中的数据由预先在大数据集上计算得到
    # 可参考wenet中compute_cmvn_stats.py的实现
    cmvn_file_path = os.path.join(get_project_dir(), config['cmvn_file'])
    mean, variance = load_cmvn(cmvn_file_path, config['is_json_cmvn'])
  ...
  encoder_type = config.get('encoder', 'conformer')
  decoder_type = config.get('decoder', 'bitransformer')
