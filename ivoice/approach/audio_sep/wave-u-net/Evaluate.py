...
def predict(track, model_config, load_model, results_dir=None):
    ...
    # 提取输入的特征，调用了getpadding的方法，进行了填充，得到了填充的特征，得到了对应需要大小的特征
    sep_input_shape, sep_output_shape = separator_class.get_padding(np.array(disc_input_shape))
    # 调用一个get——out-put的方法，这个时候还没有进行一个数据的输入
    ...
    # BUILD MODELS
    # Separator 构建分离者 四个输入参数 第一个是混合的图？？？不应该是一个input 3D的张量么
    ...
    # Upsample predicted source audio and convert to stereo. Make sure to resample back to the exact number of samples in the original input
    # (with fractional orig_sr/new_sr this causes issues otherwise)
    # 上采样预测源音频和转换为立体声。请确保重新采样到原始输入中的准确数量
    '''
    pred_audio本身的结构来说，首先是一个字典，name是他的两个名字
    '''
...
def predict_track(model_config, sess, mix_audio, mix_sr, sep_input_shape, sep_output_shape, separator_sources, mix_context):
    ...
    It iterates through the track, collecting segment-wise predictions to form the output.它遍历轨迹，收集分段预测以形成输出。
    ...
    # 如果我们的尺寸小于网络规定的输入的话，我们进行0填充，最后在结束的时候也会将0填充给去掉。
    ...
    # 可以理解为对于每一个片段的便于部分进行填充的混合，这样就可以对开始和结尾进行预测了
    ...
    '''
    这里的0可以理解为开始的时间
    source_time_frames是终止的时间
    output_time_frames是间隔
    '''
    for source_pos in range(0, source_time_frames, output_time_frames):
        ...
        # 起始 + 步长 > 最终的时间，下一个时间片段应该是最终 - 起始的那个小一点的时间片
...
def produce_source_estimates(model_config, load_model, input_path, output_path):
    '''
